...
public class LoginService {
    ...
    public ResponseEntity<Object> loginUser(LoginRequest loginRequest) {
        try {
            ...
            // that user
            //Fetch the user by Username: Dir it validate the pass ? No
            UserDetails userDetails = userDetailsService.loadUserByUsername(loginRequest.getEmailId());
            //Everything we talk about security at user means saving the username in the context we talk about only one class named SecurityContext
            // and how does the Security Context registered the user and it does this using the token
/*Note : As we know that UsernamePasswordAutnentication filter extract the usernmae and passwod from
 the request and create an authentication token and then it will pass it to the Autnetication
provider for the further validation , it will encode the passsword and do all the requeired steps
 like getting from db and password matching everythingso we need to do this here*/
            UsernamePasswordAuthenticationToken authenticationToken = new UsernamePasswordAuthenticationToken(
                    loginRequest.getEmailId(),
                    loginRequest.getPassword());


            //This guys will validate the password: we should use this below line to validate the tone as we
            ...
            //in our Security Architecture as well we learn than we pass the authenticxation token to the Authentication Provider and
            //and it will take care of the further validation
